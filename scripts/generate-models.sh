...
generate_python() {
    ...
    GENERATED_FILES=("alert.py" "alertmetadata.py" "transactionevent.py")
    ...
    for schema in "$SCHEMAS_DIR"/*.avsc; do
        ...
        echo -e "  Generating $(basename $output_file)..."
...
PYTHON_CODE
    ...
    # Update __init__.py (preserve manual exports)
...
Manual models (API-specific):
- PredictRequest, PredictResponse (in requests.py, responses.py)
- AlertDetail, AlertsResponse (in responses.py)
"""
...
# Manual models (if they exist)
try:
    from .requests import PredictRequest
    from .responses import PredictResponse, AlertDetail, HealthResponse, AlertsResponse
except ImportError:
...
EOF
    ...
    echo -e "${GREEN}✅ Python models generated in $OUTPUT_DIR${NC}"
    echo -e "${YELLOW}📝 Manual files (requests.py, responses.py) preserved${NC}"
}
